...
Render each part in order and join them. Literal text passes through; captures are looked up and substituted.
...
Parts are fully split at compile time. There is no template syntax left to parse at run time -- if you find yourself scanning for `{{`, something has gone wrong.
...
`{{@name}}` exists in the *source* language, but never reaches a payload. The compiler allocated the sentinel, so it substitutes the face while lowering and the splice arrives as ordinary text. An undeclared name is refused at compile time.

That is why there are only two kinds of part. An engine never resolves a sentinel name, and a `Program` carries faces rather than names -- see [Program](program.md).

## Watch out

- **All three capture forms refuse alike when no match anchors them** -- for instance in a template that starts a statement, where nothing was matched. That is an error, not an empty string.
...
- A sentinel's face arrives as ordinary text, and is a real character while the script runs, so later statements can match it -- that is what makes it useful as an anchor. It is cleared at exit.
