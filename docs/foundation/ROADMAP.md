...
- Registers -- the in-language spellings of the expander's metafunctions, so std bodies stay writable: `@0` reads the zero entry of the pipeline head, which is what lets `pad`'s declaration infer its fill (`{8,9,10,11,12}[pad 2]` fills with `8`; one radix rides the whole chain; an empty head degrades the fill to the unit, so `pad` no-ops -- total). Same footing as the `s` in the floor's own `{a..z}` = `{a.., !{s..}}`: notation whose expansion reads the source, never a runtime store -- captures stay on the floor's object. The register list *is* the metafunction inventory made visible, so a new register faces the scrutiny a new axiom does; the inventory is closed at four tokens -- `@` the head itself, the emitter's `$`/`$0` (Emit, below), and `$0`'s canonical read -- carrying two addressed families rather than further tokens: `$1..$n` addressing the hit's written factors, and `@lo..hi` addressing the head's value line by value, so `@0` is the degenerate `@0..0`. `where` is that family's pipeline spelling, and its digit-walk cuts by position, which is what makes the cut exact over a non-character radix where value order and shortlex part.
- Emit -- `=>` statements, two objects and one join: branches (spans carrying the floor's capture) point into text objects (the document, or a string a template constructs); a query refines a branch per match and guards on no match, a template constructs a string that commits where a branch anchors and continues a sub-branch per interpolation site, and a leading template is detached -- so the whole-document rewrite is the idiom `{@spellings} => "..."`, never a special case. The fold carries the transformation (match a later face, emit `$0`, the canonical), and `<=>` iterates a statement under a declared measure that every pass must strictly descend. A cast by value -- writing a bound value under a second universe -- is value-indexing across two radixes, which no expression computes and no register spells.
