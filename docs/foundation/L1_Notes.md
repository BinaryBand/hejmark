...
- Bounded transfinitude -- In short, while combining raw patterns can theoretically cause the language's complexity to compound exponentially, the strict tie-breaker rule acts as a geometric filter.

In some cases, it leaves the infinite structure intact ($\omega^2$); in highly ambiguous cases without distinct boundary markers, it aggressively flattens the structure back down to a simpler, finite-multiply sequence ($\omega \cdot k$).
