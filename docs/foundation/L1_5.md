...
This document specifies L1.5, the layer directly above the floor defined in [FOUNDATION.md](FOUNDATION.md). It is not L2. L1.5 adds no axiom and no meaning: every construct here is *compression* in the sense of the floor's re-admission test -- notation for a universe the floor already denotes. Meaningfulness (is a query useful, is a range sane) remains an L2 concern; L1.5 inherits the floor's faithfulness and rejects nothing.
