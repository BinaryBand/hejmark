...
- Errors live here and only here: unknown or cyclic name, malformed definition or arity, a register outside a definition body, an operand token no application binds, a matcher run against an unguarded closure body. The floor never rejects, so every rejection is an interpretation refusing a scope, never an expression failing to denote.
...
- The operand token -- `_` is provisional, and the register spellings `@`/`@0` with it; whatever the grammar picks must be legal as member and factor.
