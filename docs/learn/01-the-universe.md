# Lesson 1: The universe -- what Himark is about

## The wrong picture, and why it is wrong

If someone tells you "Himark is a pattern language," your brain immediately reaches for regular expressions. A regex like `[a-z]+` is a *predicate*: you hand it a string, it says yes or no. It is fundamentally a machine for accepting or rejecting text. Hold that picture up, look at it, and now put it down, because the mathematical floor of Himark -- the layer called L1 -- is not that. Nothing in L1 ever accepts or rejects anything. There is no text being matched. Matching is a real thing Himark does, but it lives one floor up (L1.5), and it is built *on top of* L1 the way a filesystem is built on top of a disk. L1 is the disk. It is the thing that simply *is*, before anybody asks it a question.

So what is the disk? What is the one object L1 describes?

## The one object: a pointed alphabet

L1 has exactly one kind of object, and it is called a **universe**. To build up the right picture, we need four words, and it is worth being fussy about them because the foundation document is fussy about them and the whole language turns on the distinction.
...
We close with the idea that separates Himark from every string-matching tool you know, because everything downstream depends on it.
...
You do not need to master the collision rule yet -- it gets a full treatment in lesson 3 and a whole proof file in lesson 6. Plant just this flag: **membership is about entries and their faces; a shared spelling is a collision to be adjudicated, not an identity.** The word "shortlex" you will see everywhere is simply the order used to line the spellings up so that "smaller address" has a definite meaning: shorter spellings first, and ties among equal-length spellings broken alphabetically by character code. That ordering, and why it behaves like the counting numbers $0, 1, 2, \ldots$, is the subject of the first real proof you will read.

## What you should now be able to say

- A Himark universe is a *pointed alphabet*: a dictionary of concepts (entries), each writable in one or more ways (faces), with a finger resting on one entry (`value`) and one face (`face`).
...
Next: the six constructors -- the only ways to build any of this, and the single discipline they all share.
