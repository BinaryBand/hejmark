...
This is the payoff. We read the three order-axis files line by line: `Order.lean`, `Positional.lean`, `Collision.lean`. They are the best proofs to cut your teeth on because each is self-contained (independent of the big membership-axis stack), each proves one clean theorem from lesson 3, and together they walk you up the exact ladder "finite alphabet gives $\omega$" -> "finite product gives a natural" -> "collisions are settled by a well-order."
...
That is the whole order-axis mechanization, and it is a faithful, machine-checked reflection of lesson 3's math. From here, the natural next reads are `Spelling.lean` (the gentlest membership proofs) and then `Settling.lean` (the hardest), now that you know the shape of the tree and the tool.
