...
You do not need to reproduce these computations. The takeaway: L1's universes are transfinite but *bounded*, the bound is $\varepsilon_0$, and how high a given expression climbs is a precise function of how its closures nest. Lesson 6's order-axis proofs mechanize the *bottom* of this ladder rigorously (finite factors give naturals, positional value is mixed radix); the full climb to $\varepsilon_0$ is the one genuinely research-grade piece still deferred in the Lean development, and now you know why it is hard.
