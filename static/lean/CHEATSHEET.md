...
<br>`| n+1 => ...` | define a recursive function by pattern matching |
...
| `first | t1 | t2` | try `t1`; if it fails, try `t2` (also written as `t1 < |
...
* **`Prop` elimination is restricted.** You cannot pattern-match a `Prop` (like `∃` or `∨`) to construct a value in `Type` unless it is a singleton/empty proposition. This is critical for proof erasure.
