...
5. Migrate `entriesType` -> `prodLt` -> `unionLt` -> `Rows` onto `entryRecLt`, each a green build, deleting each approximation once its replacement is proved. What survives the migration: `entrySpellLt` / `entriesType` as the leaf primitives the recursion itself bottoms out on (and the Phase-A theorems about them, which are true of the spelling order specifically), and `entryLt` as the 4b within-stage nested-closure fallback. What gets deleted: the `prodLt` and `unionLt` approximation orders with their isos and type theorems, once the same order-type laws are proved of `entryRecLt` directly and the rows are restated over it (the `HEADLINE_THEOREMS` entries for the superseded statements swap to their replacements at that point). Itself sliced:
   ...
   - 5c. The union law on the recursive order: the cons-spine block law (first-owner ownership, `mBound` offset past the head block, `nRank_cons_first` / `nRank_cons_rest`) assembled into an iso from `entryRecLt (napp n1 n2)` onto the lex sum of the first body and the second body's unclaimed remainder -- the recursive analogue of `unionSumIso` -- with the ordinal-sum type theorem and its disjoint corollary, the replacements for `unionLt_type` / `unionLt_type_disjoint`.
   - 5d. Closure enumeration types: the Phase-E analogue over the recursive order -- a binder with finite stages has `entryRecType` at most omega (an `entryRecLt`-predecessor appears no later, by the contrapositive of `cRank_lt_of_firstStage_lt`, so predecessors live inside one finite stage), exactly omega when the denotation is infinite; specialized to `entryRecType (unitClosure lo hi) = omega`, what the migrated rows read where they now read `unitClosure_entriesType`.
   - 5e. Rows migration and deletion: the five row theorems restated over `entryRecLt` / `entryRecType` (union row at $\omega + 2$, two-blocks row at $\omega \cdot 2$, seam row at $\omega^2$, collapse row at $\omega$, nonempty collapse at $\omega \cdot 2$), then delete the superseded approximations (`prodLt` / `prodAddrEmb` / `prodSplitIso` / `prodLt_type_of_unique_splits`; `unionLt` / `unionAddrEmb` / `unionSumIso` / `unionLt_type` / `unionLt_type_disjoint`; the old row statements) and swap the corresponding `HEADLINE_THEOREMS` entries.
