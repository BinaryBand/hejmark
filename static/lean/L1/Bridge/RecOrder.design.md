...
5. Migrate `entriesType` -> `prodLt` -> `unionLt` -> `Rows` onto `entryRecLt`, each a green build, deleting each approximation once its replacement is proved. What survives the migration: `entrySpellLt` / `entriesType` as the leaf primitives the recursion itself bottoms out on (and the Phase-A theorems about them, which are true of the spelling order specifically), and `entryLt` as the 4b within-stage nested-closure fallback. What gets deleted: the `prodLt` and `unionLt` approximation orders with their isos and type theorems, once the same order-type laws are proved of `entryRecLt` directly and the rows are restated over it (the `HEADLINE_THEOREMS` entries for the superseded statements swap to their replacements at that point). Itself sliced:
   ...
   - 5b. The product law on the recursive order: for `prod2 a b` (no literal `&` factor, so the rank runs the mixed-radix branch) with unique splits, `entryRecLt (prod2 a b)` is order-isomorphic to `Prod.Lex (entryRecLt a) (entryRecLt b)` -- the recursive analogue of `prodSplitIso`, read off `fRank_node_nb` with `someSplit` pinned to the unique split; needs the comparison form of the mixed-radix arithmetic (`mixmul_lt_iff` alongside `mixmul_lt` / `mixmul_inj`). Corollary: `Ordinal.type (entryRecLt (prod2 a b)) = entryRecType b * entryRecType a`, the replacement for `prodLt_type_of_unique_splits`.
