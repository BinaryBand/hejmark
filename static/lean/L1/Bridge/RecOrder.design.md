...
4. Closure within-stage body-recursion (own increment), itself sliced:
   ...
   - 4c. Tie the knot: define the closure rank by well-founded recursion on the stage index -- stage `k+1`'s within-rank is 4b's body rank with amp-rank set to the closure rank restricted to stages `<= k` (mediated by `firstStage`); the earlier-stage rank is strictly smaller, so the recursion is well-founded.
