...
4. Closure within-stage body-recursion (own increment), itself sliced:
   ...
   - 4d. Closure faithfulness + swap: assemble injectivity via 4a fed by 4b/4c and the compositional bound (the sup over stages, the closure's order type), then replace the `typein (entryLt node)` fallback branches in `mRank` / `nRank` / `fRank` for binder nodes, fold-of-binder, and amp-products, extending `entryRecLt_isWellOrder` past the non-binder skeleton. Itself sliced:
     ...
     - 4d-ii. Entry-level total-bound closure faithfulness: define `cBound n` (the sup over stages `⨆ k, csBound n k`), prove cross-stage stabilization (`csRank n k s = csRank n (firstStage n s) s` for `firstStage n s ≤ k`), and `Faithful (cRank n) (cBound n)` for a genuine binder node (`bindsb n = true`), plus the fold-of-binder reinterpretation routing an `nsingle (.fold inner)` entry to `cRank inner`. Purely additive, no swap.
     - 4d-iii. The swap: replace the `typein (entryLt node)` fallback branches (and `closureBound` bounds) in `mRank` / `nRank` / `fRank` with `cRank` / `cBound`, and re-prove `mFaithful` / `nFaithful` / `fFaithful` and `entryRecLt_isWellOrder` with the closure cases discharged by 4d-ii instead of `faithful_typein`. Invasive to the landed step-2 definitions.
