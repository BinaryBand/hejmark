...
> **Completed design record, not a live spec.** Every step below has landed; the record is kept for the reasoning, not as a description of current code. Names it discusses as present or planned -- `closureBound`, `prodLt`, `unionLt`, the abstract stage-major backbone (`stageOffset` / `stageRank`) and kin -- no longer exist in the tree. For what is true now, and for what remains deferred, see `static/lean/README.md`.
...
--   within-stage body-recursion replaced this fallback in WP1; no closure case
--   was ever stubbed.
...
Staging (revised): the first increment ranks *every* closure -- a top-level binder node and a fold-of-binder alike -- by `typein (entryLt node)`, the existing stage-major/shortlex-within-stage well order. That order is already proven (Entries.lean) and already faithful at *stage* granularity (the demotion-row theorems are at exactly that resolution), and its `typein` is an injective rank for free -- so the closure case is *complete*, not stubbed, from increment one. The body-recursion runs only on the genuine non-binder skeleton (leaf / union / product / fold-of-non-binder). Promoting the within-stage order from this `entryLt` fallback to a body-recursive order was the dedicated follow-up increment; it has landed (WP1 below). While the fallback stood, `entryRecLt` and `entryLt` agreed on every closure by construction; now that it is gone the same guarantee is carried explicitly, by `entryRank_promotion_agree` on every node with no closure nested below it.
...
1. [landed] Prove injectivity + the bound invariant -> `IsWellOrder (entryRecLt n)`, for leaf + union + product + fold (`entryRecLt_isWellOrder`, over the deep subtraction-free skeleton `nSubfree`).
1. [landed] Prove the leaf agreement bridge lemma (`entryRecLt_leaf`, specialized to `entryRecLt_face` / `_range` / `_final`). Import `RecOrder.lean` into `L1.lean`; gate stays green (`entryRecLt_isWellOrder` and `entryRecLt_face` added to `HEADLINE_THEOREMS`, axioms clean).
1. Closure within-stage body-recursion (own increment), itself sliced:
   ...
   - 4d. [landed] Closure faithfulness + swap: assemble injectivity via 4a fed by 4b/4c and the compositional bound (the sup over stages, the closure's order type), then replace the `typein (entryLt node)` fallback branches in `mRank` / `nRank` / `fRank` for binder nodes, fold-of-binder, and amp-products, extending `entryRecLt_isWellOrder` past the non-binder skeleton. Itself sliced:
     ...
     - 4d-iii. [landed] The swap: the step-2 assembly (the mutual `mRank` / `nRank` / `fRank` / `mBound` / `nBound` / `fBound`, `entryRank` / `entryRecLt` and the free instances, the unfolding lemmas, the faithfulness mutual, the payoff, and leaf agreement) relocated below the closure machinery it now consumes, and the six closure fallback branches (`mRank` on `.amp`, fold-of-binder, and amp-product; `nRank` on a binder spine; `fRank` on `.amp rest` and a binder-tail `.node`) replaced by `cRank` with bounds `cBound` -- the fold-of-binder branch a dite on `bindsb inner` routed through `foldBinderReinterp` to the inner closure's `cRank` / `cBound`. `mFaithful` / `nFaithful` / `fFaithful` re-proved with the closure cases discharged by `cFaithful` / `foldBinderFaithful` instead of `faithful_typein`; `entryRecLt_isWellOrder`, the payoff, and the leaf-agreement statements unchanged in form, now over the genuinely body-recursive closure rank. `closureBound` survived at this stage solely as the 4b within-stage recursion's nested-closure fallback bound; WP1 deleted it.
1. Migrate `entriesType` -> `prodLt` -> `unionLt` -> `Rows` onto `entryRecLt`, each a green build, deleting each approximation once its replacement is proved. What survives the migration: `entrySpellLt` / `entriesType` as the leaf primitives the recursion itself bottoms out on (and the Phase-A theorems about them, which are true of the spelling order specifically), and `entryLt` as the 4b within-stage nested-closure fallback (retired in turn by WP1, leaving `entryLt` a Phase-E order in `Entries.lean` with no role in the recursive order). What gets deleted: the `prodLt` and `unionLt` approximation orders with their isos and type theorems, once the same order-type laws are proved of `entryRecLt` directly and the rows are restated over it (the `HEADLINE_THEOREMS` entries for the superseded statements swap to their replacements at that point). Itself sliced:
   - 5a. [landed] The recursive entries enumeration `entryRecType` (the order type of `entryRecLt`, dite-guarded on the subtraction-free skeleton where `entryRecLt_isWellOrder` supplies the instance, junk `0` off it), its unfolding `entryRecType_def`, and leaf agreement `entryRecType_leaf` (+ `_face` / `_range` / `_final` specializations): on a leaf member the recursive enumeration is the spelling enumeration, step 3's `entryRecLt_leaf` read through a reflexive order isomorphism. Purely additive, in `RecOrder.lean`.
   - 5b. [landed] The product law on the recursive order: for `prod2 a b` (no literal `&` factor, so the rank runs the mixed-radix branch) with unique splits and subtraction-free factors, `prod2RecIso : entryRecLt (prod2 a b)` is order-isomorphic to `Prod.Lex (entryRecLt a) (entryRecLt b)` -- the recursive analogue of `prodSplitIso`, with the recursion's own split choice as the pieces map (`prod2RecPieces`), pinned to the unique split by `prod2_someSplit_eq` and forced to `(q, [])` on the singleton tail by `someSplit_prodNil_eq` (no uniqueness needed there). The rank collapses to closed mixed-radix form (`entryRank_prod2`, via `fRank_node_nil` absorbing the constant `prodNilRank` pad of the forced-empty tail-of-tail), and the comparison form of the mixed-radix arithmetic (`mixmul_lt_iff` alongside `mixmul_lt` / `mixmul_inj`) turns faithfulness into the lex comparison (`entryRecLt_prod2_iff`). Corollary `entryRecType_prod2 : entryRecType (prod2 a b) = entryRecType b * entryRecType a`, the replacement for `prodLt_type_of_unique_splits`.
