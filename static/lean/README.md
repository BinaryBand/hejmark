...
- **Unsettled closures**: the Python raises `HimarkUnsettledError` where `containsb` answers `false` at the stage bound, so the evaluator under-approximates on unsettled bodies. Exactness on settled bodies is the **guarded settling theorem**: for guarded bodies, `(exists k, stage n k s) <-> stage n (length s + 1) s`. The hard `->` direction is now proved in `L1/Settling.lean` (`guarded_settles`; the `<-` direction is stage monotonicity), so the doc's length-bounded stage and the Python `length s + 1` bound are mechanized. Two pieces still stand between this and full evaluator exactness: the boolean bridge `settledb n = true -> SemSettled n` (so the theorem applies to the evaluator's own checker), and lifting soundness (`containsb_sound`) to a two-sided `iff` on the settled fragment.
