...
second mechanization of L1. It carries one ported theorem as a trial (see below); the rest of
the membership axis stays in Coq until/unless a decision is made to migrate or fork it.
...
- `L1/Spelling.lean` -- name mirrors `static/formal/L1/Spelling.v`. Carries a minimal
  `lexLt`/`shortlexLt` and one ported theorem, `singleton_shortlexLt_iff` (the Lean analogue of
  Coq's `singleton_ltb_iff`), as a workflow trial: `simp` alone discharges what Coq needed
  `orb_true_iff`/`andb_true_iff`/`Nat.ltb_lt` bookkeeping for.
