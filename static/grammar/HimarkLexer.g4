...
// everything else, whitespace and newlines included, is a literal face
// character -- a space is a space, so `{cat dog}` is one face and members are
// separated by `,` alone (`{cat,dog}`). Apply a definition with `@name(args)`,
// never by putting a space between the name and its argument.
// ---------------------------------------------------------------------------
...
B_LPAREN : '('  -> type(LPAREN), pushMode(ARGS) ;
...
// ---------------------------------------------------------------------------
// ARGS mode -- inside a pipeline bracket `[...]` or an application `(...)`.
