...
AST rather than by denoting anything, which keeps this module a leaf:
...
lengthens and membership settles at stage ``len(spelling) + 1``? A bare ``&`` is
unguarded (the pass can reproduce itself); a ``&`` in a product is guarded when
some sibling factor cannot spell the empty face. This is what L2's unsettled
refusal reads: inside the guarded fragment absence is decided, outside it
absence has no bound.
...
keeps the module a leaf, and it states the real shape of the property --
